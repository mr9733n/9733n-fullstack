        ...
        async with aiohttp.ClientSession() as session:
            while retries < max_retries:
                try:
                    ...
                    logger.debug(f"Fetched numbers for {country}: {numbers}")
                    if numbers:
                        return numbers
                    else:
                        logger.warning(f"No numbers fetched for {country}.")
                        return None
                except Exception as e:
                    logger.error(f"Error fetching numbers for {country}: {e}")
                retries += 1
                await asyncio.sleep(retry_delay)
                retry_delay *= 2
        logger.error(f"Failed to fetch numbers for {country} after {max_retries} retries.")
        return None

    async def fetch_numbers_for_country(self, session, country):
        try:
            logger.info(f"Utils: Fetching numbers for country: {country}")
            fresh_numbers = await self.onlinesim_helper.fetch_numbers(session, country)
            logger.debug(f"Fetched numbers before sorting: {fresh_numbers}")

            if fresh_numbers:
                sorted_numbers = self.onlinesim_helper.sort_numbers(fresh_numbers)
                logger.debug(f"Sorted numbers: {sorted_numbers}")

            # Обновляем глобальный кэш
            cache["data"][country] = sorted_numbers
            cache["last_update"] = asyncio.get_event_loop().time()
            logger.info(f"Utils: Updated cache for {country}")
        except aiohttp.ClientTimeout:
            logger.error(f"Utils: Timeout while fetching numbers for country: {country}.")
        except ssl.SSLError:
            logger.error(f"Utils: SSL error while fetching numbers for country: {country}.")
        except Exception as e:
            logger.error(f"Utils: Unexpected error fetching numbers for country {country}: {e}")
